...
perf_parallel_join 75: lowest(4 secs, 262 ms, 494 μs, and 1 hnsec) total(43 secs, 32 ms, 47 μs, and 4 hnsecs)
perf_parallel_join 78: lowest(4 secs, 274 ms, 481 μs, and 4 hnsecs) total(43 secs, 55 ms, 478 μs, and 2 hnsecs)
```
...
After some study of `Relation.from` and thinking of the use cases for it I concluded that "no". This is not the right place to add the parallel sort. It is because then it would only cover those cases where `from` is used.

What about the constructor itself?
...
There is a parallel sort example in [std.parallelism](https://dlang.org/phobos/std_parallelism.html#.task). I do not see any reason to try to duplicate the effort there so I chose to copy it as is into the constructor.

So far we have now made `Iteration`, `Variable` and `Relation` aware of `ThreadStrategy`. The last piece of the puzzle is to make `datacat.join.join` create sorted `Relation`s using the thread strategy of the `Variable`. This where achieved by adding a new template parameter, `ThreadStrategy TS`, that propagates `TS` to the contructor of `Relation`.

What is the end result when running the coarse grained benchmarks?
```sh
perf_parallel_join 75: lowest(414 ms, 95 μs, and 1 hnsec) total(4 secs, 249 ms, 611 μs, and 4 hnsecs)
perf_parallel_join 78: lowest(353 ms, 364 μs, and 8 hnsecs) total(3 secs, 913 ms, 818 μs, and 2 hnsecs)
```
...
Which map well to the coarse grained test that showed a 15% improvement.

This my folk is why it is so important to have a test suite.

Yes, multithreading helped in this case. Not by a lot but an improvement is an improvement.

The answer to the question at the beginning is thus **yes**.
