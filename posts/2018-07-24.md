...
The common aspect that they all end up doing is *inserting* the result into the output. Wouldn't that be the perfect place to put the parallel sort? In the insert? As an additional bonus it would mean that any new functions would benefit from the improved sort.
...
No noticeable change. The difference is within the margin of error.
...
Now **this** is surprising. Simplifying the code and it doubled the performance. I do not yet know why this happened.

I am a bit sceptically of these numbers so to be sure I have verified the number of computed nodes with DataFrog. They are the same.
